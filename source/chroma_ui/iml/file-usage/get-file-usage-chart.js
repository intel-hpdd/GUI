...
angular.module('fileUsageModule')
  .factory('getFileUsageChart',
    function getFileUsageChartFactory (resolveStream, getFileUsageStream, DURATIONS, chartPlugins,
      ...
      return function getFileUsageChart (title, keyName, overrides) {
        ...
        return chartCompiler('iml/file-usage/assets/html/file-usage-chart.html', {
          title: title,
          configure: false,
          stream: resolveStream(getFileUsageStream(
            keyName,
            requestDuration,
            chartPlugins.bufferDataNewerThan.apply(null, DEFAULT_DURATION)
          )),
          onSubmit: function onSubmit (fileUsageForm) {
            this.stream.destroy();

            var requestRange, buff;

            if (fileUsageForm.rangeForm) {
              var start = fileUsageForm.rangeForm.start.$modelValue;
              var end = fileUsageForm.rangeForm.end.$modelValue;

              requestRange = getTimeParams.getRequestRange(start, end);
              buff = fp.identity;
            } else if (fileUsageForm.durationForm) {
              var size = fileUsageForm.durationForm.size.$modelValue;
              var unit = fileUsageForm.durationForm.unit.$modelValue;
            ...
            this.stream = getFileUsageStream(keyName, requestRange, buff);
            this.onCancel();
          },
