...
angular.module('dashboard')
  .controller('DashboardCtrl', function DashboardCtrl ($scope, $location, $routeSegment,
    ...
    fsStream
      .tap(fp.lensProp('fileSystems').set(fp.__, dashboard))
      .stopOnError($scope.handleException)
      .each($scope.localApply.bind(null, $scope));

    hostStream
      .pluck('objects')
      .tap(fp.lensProp('hosts').set(fp.__, dashboard))
      .stopOnError($scope.handleException)
      .each($scope.localApply.bind(null, $scope));
    ...
    function setData (params) {
      if (params.fsId) {
        ...
        fsStream2.flatMap(highland.findWhere({ id: params.fsId }))
          .tap(fp.lensProp('fsData').set(fp.__, dashboard))
        ...
        hostStream2
          ...
          .tap(fp.lensProp('hostData').set(fp.__, dashboard))
          .stopOnError(fp.curry(1, $scope.handleException))
          .each($scope.localApply.bind(null, $scope));
      ...
      if (params.targetId) {
        ...
        targetStream2
          ...
          .tap(fp.lensProp('targetData').set(fp.__, dashboard))
          .stopOnError($scope.handleException)
          .each($scope.localApply.bind(null, $scope));
      }
    }
