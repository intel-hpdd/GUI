...
angular.module('action-dropdown-module')
  .controller('ConfirmActionModalCtrl', ['$scope', 'title', 'confirmPrompts',
    function ConfirmActionModalCtrl  ($scope, title, confirmPrompts) {
      'use strict';
  ...
  .factory('openConfirmActionModal', ['$modal', function openConfirmActionModalFactory ($modal) {
    ...
    return function openConfirmActionModal (title, confirmPrompts) {
      return $modal.open({
        ...
        backdrop: 'static',
        resolve: {
          title: _.fidentity(title),
          confirmPrompts: _.fidentity(confirmPrompts)
        }
      });
    };
  }]);
