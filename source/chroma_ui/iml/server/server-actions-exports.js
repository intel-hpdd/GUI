...
export default function serverActionsFactory () {
  ...
  function convertNonMultiJob (hosts) {
    ...
    return [{
      ...
      args: {
        hosts: hosts.map(lensProp('resource_uri'))
      }
    }];
  }

  const noUpdates = eqFn(identity, lensProp('needs_update'), false);
  const memberOfFs = eqFn(identity, lensProp('member_of_active_filesystem'), true);
  const mutableState = eqFn(identity, lensProp('immutable_state'), false);
 ...
 Stop associated active file system(s) to install updates.`],
  ...
  const isNotManagedServer = eqFn(identity, pathLens(['server_profile', 'managed']), false);
