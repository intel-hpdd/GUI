...
const getLocks = (type: ReadOrWriteT) => (contentTypeId: number, recordId: number, locks: LockT) => {
  return flattenLocks(locks).filter(
    (x: LockEntryT) => x.lock_type === type && x.content_type_id === contentTypeId && x.item_id === recordId
  );
};

export type GetTypeLocksT = (contentTypeId: number, recordId: number, locks: LockT) => LockEntryT[];
