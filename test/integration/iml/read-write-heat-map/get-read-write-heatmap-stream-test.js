...
import readWriteHeatMapDataFixtures
  from '../../../data-fixtures/read-write-heat-map-fixtures.json!json';
...
describe('the read write heat map stream', () => {
  ...
  beforeEachAsync(async function() {
    ...
    socketStream = jasmine.createSpy('socketStream').and.callFake(path => {
      ...
      if (path === '/target/metric') streams.heatMap.push(s);
      else if (path === '/target') streams.target.push(s);
  ...
  it('should return a factory function', () => {
    expect(getReadWriteHeatMapStream).toEqual(jasmine.any(Function));
  });
  ...
  describe('fetching 10 minutes ago', () => {
    ...
    beforeEach(() => {
      ...
      readWriteHeatMapStream = getReadWriteHeatMapStream(
        'stats_read_bytes',
        requestDuration,
        buff
      );

      readWriteHeatMapStream.each(spy);
    });
    ...
    describe('when there is data', () => {
      ...
      it('should return a stream', () => {
        expect(highland.isStream(readWriteHeatMapStream)).toBe(true);
      });
      ...
      it('should maintain consistent data', () => {
        ...
        expect(spy).toHaveBeenCalledWith(fixtures[0].out);
      });

      it('should union with a target', () => {
        streams.heatMap[1].write({
          1: fp.view(fp.lensProp(1))(clone(fixtures[0].in))
        });
        streams.heatMap[1].end();
