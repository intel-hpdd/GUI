...
describe('pathMaxLength Filter', () => {
  ...
  beforeEach(
    angular.mock.inject(function($cacheFactory) {
  ...
  const testCases = {
    '/yay/fun.txt': [
      {
        maxLength: 60,
        result: '/yay/fun.txt',
        msg: 'should leave this path alone',
        cached: false
      },
      {
        maxLength: 10,
        result: '...',
        msg: 'should reduce this to just ...',
        cached: true
      }
    ],
    '/foo/remove_me/baz/fun.txt': [
      {
        maxLength: 20,
        result: '/foo/.../baz/fun.txt',
        msg: 'should be exactly 20 chars',
        cached: true
      },
      {
        maxLength: 16,
        result: '/foo/.../fun.txt',
        msg: 'should prefer left to right side',
        cached: true
      },
      {
        maxLength: 15,
        result: '/.../fun.txt',
        msg: 'should be only the filename and ... dir',
        cached: true
      }
    ],
    ...
    '/foo/bar/really_long_file_name_here.txt': [
      {
        maxLength: 39,
        result: '/foo/bar/really_long_file_name_here.txt',
        cached: false
      },
      {
        maxLength: 35,
        result: '/.../really_long_file_name_here.txt',
        cached: true
      },
    ...
    'no/leading/slash/file.txt': [
      {
        maxLength: 25,
        result: 'no/leading/slash/file.txt',
  ...
  Object.keys(testCases).forEach(function(path) {
    testCases[path].forEach(function(test) {
      const msg = test.msg
        ? test.msg
        : `should change ${path} to ${test.result}`;
