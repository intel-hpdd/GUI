...
describe('user subscriptions component', () => {
  ...
  beforeEach(module('extendScope', 'bigDifferModule', 'configToggle', 'multiToggler', ($provide, $compileProvider) => {
    $provide.value('insertHelpFilter', jasmine.createSpy('insertHelpFilter'));
  ...
  it('should have a header', () => {
    expect(el.querySelector('.section-header'))
      .toHaveText('Notifications');
  });

  it('should show the configure button', () => {
    expect(el.querySelector('.edit-btn'))
      .not
      .toBeNull();
  });
  ...
  it('should show the list of choices', () => {
    const labels = Array
      .from(el.querySelectorAll('.sub-label'))
      .map(x => x.textContent);


    expect(labels)
      .toEqual(['Alert event:', 'Syslog event:']);
  });

  it('should not show the saving banner', () => {
    expect(el.querySelector('.alert-info'))
      .toBeNull();
  });
  ...
  describe('configuring', () => {
    ...
    it('should show the toggle controls', () => {
      expect(el.querySelector('.toggle-controls'))
        .not
        .toBeNull();
    });
    ...
    it('should update on change', () => {
      ...
      expect(el.querySelectorAll('.sub-label').length)
        .toBe(0);
    });
    ...
    describe('saving', () => {
      ...
      it('should patch with the right data', () => {
        expect(socketStream)
          .toHaveBeenCalledOnceWith('/api/alert_subscription/', {
            method: 'patch',
            json: {
              objects: [
                {
